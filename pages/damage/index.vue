      ...
      <div class="kt-portlet__head-toolbar">
        <div class="kt-portlet__head-wrapper">
          <div class="kt-portlet__head-actions">
            ...
            >
              <i class="la la-plus" />
              <span class="kt-hidden-mobile">Add Damage</span>
            </a>
          </div>
        </div>
      </div>
...
export default {
  ...
  async mounted () {
    ...
    // begin first table
    this.datatable = $('#damage_table').DataTable({
      responsive: true,
      searching : false,
      processing: true,
      serverSide: true,
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render   : function (data, type, full, meta) {
            ...
            return `<a href="/damage/detail/${btoa(full.id)}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="View Details">
                      <i class="la la-eye"></i>
                    </a>
                    ${actionButtonEdit}
                    <span class="dropdown">
  ...
  methods: {
    ...
    async setStatus (statusId, row) {
      ...
      for (const statusIndex in JOB_STATUS) {
        if (statusId === JOB_STATUS[statusIndex].id) {
          ...
          swal.fire({
            title             : 'Are you sure?',
            text              : `Job damage "${row.job_no}" will be ${statusText}`,
            type              : 'question',
    ...
    async updateStatus (idDamage, statusId, param) {
      try {
        this.$nuxt.$loading.start()
        param.status    = statusId
        this.$delete(param, 'job_close_date')
        await this.$store.dispatch('damage/editDamage', { idDamage: idDamage, data: param })
        const data      = this.$store.getters['damage/getEditDamage']
        const parameter = {
          alertClass: 'alert-success',
          message   : `Job Damage transfer ${data.result.job_no} has been edited`,
        }
